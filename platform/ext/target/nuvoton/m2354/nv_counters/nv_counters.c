...
/*
    return value of zero's counts but the data itself
*/
        ...
        uint32_t size, uint8_t *val)
{
    ...
    for(byte_idx = 0; byte_idx < counter_size; byte_idx++)
    {
        ...
        {
            count += (((counter_value[byte_idx] >> bit_idx) & 1) == 0) ? 1 : 0;
        }
    }
...
/*
    Translate value to coutns of zero's.
    Then write to OTP
*/
        ...
        uint32_t value)
{
    ...
    /* Translate value to counts of zero's */
    memset(counter_value, 0xff, OTP_COUNTER_MAX_SIZE);
    for(byte_idx = 0; byte_idx < (value / 8); byte_idx++)
    {
        counter_value[byte_idx] = 0;
    }
